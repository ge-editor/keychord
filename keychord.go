package keychord

import (
	"errors"
	"fmt"
	"sort"
	"strings"
	"unicode"
	"unicode/utf8"

	"github.com/gdamore/tcell/v3"

	"github.com/ge-editor/gelog"
)

type KeySpec struct {
	Key tcell.Key
	Str string
	Mod tcell.ModMask
}

var (
	ErrInvalidKeyEvent = errors.New("error invalid key event")
)

type DecodedKey struct {
	Key tcell.Key     // 特殊キー
	Str string        // printable 文字
	Mod tcell.ModMask // ctrl / alt / shift
}

// KeySpec を返すユーティリティ
func (d DecodedKey) KeySpec() KeySpec {
	return KeySpec{
		Key: d.Key,
		Str: d.Str,
		Mod: d.Mod,
	}
}

func Decode(s string) (DecodedKey, error) {
	var out DecodedKey
	if s == "" {
		gelog.Key("[Decode:invalid]", "reason", "empty", "s", s)
		return out, ErrInvalidKeyEvent
	}

	parts := strings.Split(s, "+")
	var keyPart string

	for _, p := range parts {
		pl := strings.ToLower(p)
		switch pl {
		case "ctrl":
			out.Mod |= tcell.ModCtrl
		case "alt":
			out.Mod |= tcell.ModAlt
		case "shift":
			out.Mod |= tcell.ModShift
		case "meta":
			out.Mod |= tcell.ModMeta
		default:
			keyPart = p
		}
	}

	if keyPart == "" {
		// gelog.Key("keyPart is blank, invalid", "keyPart", keyPart, "s", s)
		return out, ErrInvalidKeyEvent
	}

	// Specila Keys
	// "Delete", "Esc", ... tcell/v3 の key.go を見てね
	for k, name := range tcell.KeyNames {
		if strings.EqualFold(keyPart, name) {
			// gelog.Key("Special key", "s", s, "KeyName", name)
			out.Key = k
			return out, nil
		}
	}

	// 0 	000 0000 	000 	0x00 	NUL ^@ 	Null
	//
	// 27 	001 1011 	033 	0x1b 	ESC	^[ 	Escape
	// 28 	001 1100 	034 	0x1c 	FS 	^\ 	File Separator
	// 29 	001 1101 	035 	0x1d 	GS 	^] 	Group Separator
	// 30 	001 1110 	036 	0x1e 	RS 	^^ 	Record Separator
	// 31 	001 1111 	037 	0x1f 	US 	^_ 	Unit Separator
	// 32 	010 0000 	040 	0x20 	SP 	^@	Space
	//
	// 33-47	! - /
	// 48-57	0-9		Number
	//
	// 127 	111 1111 	177 	0x7f 	DEL 	^? 	delete character

	// 2) printable: KeyRune
	if len(keyPart) == 1 {
		r := rune(keyPart[0])

		// Ctrl+A - Ctrl+Z, ^@, ^[, ^\, ^], ^^, ^_
		if out.Mod&tcell.ModCtrl != 0 {
			lower := unicode.ToLower(r)
			code := tcell.Key(r) - '@'
			if code == 27 { // ^[
				out = DecodedKey{Key: code}
				return out, nil
			}
			if code == 0 { // ^@ (^Space)
				// gelog.Key("^@ (^Space)", "r", r, "s", s, "code", code, "str", out.Str, "key", out.Key, "spec", out.KeySpec())
				out = DecodedKey{Key: 256, Str: " ", Mod: tcell.ModCtrl}
				return out, nil
			}
			if code >= 28 && code <= 31 { // ^\, ^], ^^, ^_ (^/)
				// gelog.Key("code: 28 to 31", "r", r, "s", s, "code", code, "str", out.Str, "key", out.Key, "spec", out.KeySpec())
				out = DecodedKey{Key: 256, Str: string(r), Mod: tcell.ModCtrl}
				return out, nil
			}
			if lower >= 'a' && lower <= 'z' {
				out.Key = tcell.Key(lower - 'a' + rune(tcell.KeyCtrlA))
				// gelog.Key("Ctrl+A to Ctrl+Z", "r", r, "s", s, "code", code, "str", out.Str, "key", out.Key, "spec", out.KeySpec())
				return out, nil
			}
			// Ctrl + 非ASCII → 無効
			// key: 0
			// gelog.Key("Ctrl + Non ASCII: invalid", "r", r, "s", s, "code", code, "str", out.Str, "key", out.Key, "spec", out.KeySpec())
			return out, ErrInvalidKeyEvent
		}

		// Rune 普通の文字
		out.Key = tcell.KeyRune
		out.Str = keyPart
		// gelog.Key("Rune (standard string)", "s", s, "str", out.Str)
		return out, nil
	}

	// 3) Unicode 1文字
	if utf8.RuneCountInString(keyPart) == 1 {
		out.Key = tcell.KeyRune
		out.Str = keyPart
		// gelog.Key("Unicode (a charactor)", "s", s, "str", out.Str)
		return out, nil
	}

	// gelog.Key("Invalid", "s", s)
	return out, ErrInvalidKeyEvent
}

type RootNode struct {
	root      *KeyNode
	current   *KeyNode
	keyStatus string
	keyEvent  func(tcell.EventKey) (string, KeyDispatchTransition) // ユーザー独自のキーイベント関数が必要な場合
}

var rootNodes []*RootNode

func ResetAllRootNodes() {
	gelog.Key("[KeyDispatch:ResetAllRootNodes]",
		"count", len(rootNodes),
	)

	for i, r := range rootNodes {
		gelog.Key("[KeyDispatch:ResetRoot]",
			"index", i,
		)

		r.Reset()
	}
}

// NewRootNode
func NewRootNode() *RootNode {
	key := newKeyNode()
	root := &RootNode{
		root:    key,
		current: key,
	}
	rootNodes = append(rootNodes, root)
	return root
}

func (r *RootNode) BindKeyEvent(f func(tcell.EventKey) (string, KeyDispatchTransition)) {
	r.keyEvent = f
}

// bindKeys は KeyStr 配列に対してアクションを登録する
// action は DispatchResult を返すように変更して非同期 / Blocking を制御可能
// func (r *RootNode) bindKeys(keyStrs []string, action func()) error {
func (r *RootNode) bindKeys(keyStrs []string, action KeyAction) error {
	if len(keyStrs) == 0 {
		return errors.New("empty key sequence")
	}
	node := r.root
	for i, ks := range keyStrs {
		k, err := Decode(ks)
		if err != nil {
			return fmt.Errorf("invalid key %q: %w", ks, err)
		}
		if node.Next == nil {
			node.Next = make(map[KeySpec]*KeyNode)
		}
		next, ok := node.Next[k.KeySpec()]
		if !ok {
			next = newKeyNode()
			node.Next[k.KeySpec()] = next
		}

		// KeyStr をセット（作成時のみ）
		if next.KeyStr == "" {
			next.KeyStr = ks
		}
		if i == len(keyStrs)-1 {
			next.Action = action
		}
		node = next
	}
	return nil
}

// Reset 現在のノードを初期化
func (r *RootNode) Reset() {
	r.current = r.root
	r.keyStatus = ""
}

var dispatchSeq uint64

func nextDispatchID() uint64 {
	dispatchSeq++
	return dispatchSeq
}

type KeyDispatchTransition int

const (
	DispatchNotFound           KeyDispatchTransition = iota // 該当するキーがない
	DispatchPrefix                                          // プレフィックスキーで次の入力を待つ
	DispatchExecuted                                        // アクション実行済み
	DispatchInvalidAfterPrefix                              // プレフィックス後に無効なキー
)

func (r KeyDispatchTransition) String() string {
	switch r {
	case DispatchNotFound:
		return "DispatchNotFound"
	case DispatchPrefix:
		return "DispatchPrefix"
	case DispatchExecuted:
		return "DispatchExecuted"
	case DispatchInvalidAfterPrefix:
		return "DispatchInvalidAfterPrefix"
	default:
		return fmt.Sprintf("DispatchTransition(%d)", int(r))
	}
}

func nodeName(n *KeyNode) string {
	if n == nil {
		return "<nil>"
	}
	if n.KeyStr == "" {
		return "<root>"
	}
	return n.KeyStr
}

// Dispatch 入力イベントに応じてキーバインドを実行
func (r *RootNode) Dispatch(ev tcell.EventKey) (string, KeyDispatchTransition) {
	id := nextDispatchID()

	gelog.Key("[KeyDispatch:start]",
		"id", id,
		"key", ev.Key(),
		"str", ev.Str(),
		"mod", ev.Modifiers(),
		"current", nodeName(r.current),
	)

	defer func() {
		gelog.Key("[KeyDispatch:end]",
			"id", id,
			"current", nodeName(r.current),
			"status", r.keyStatus,
		)
	}()

	k := KeySpec{
		Key: ev.Key(),
		Str: ev.Str(),
		Mod: ev.Modifiers(),
	}

	if r.current == nil {
		r.current = r.root
	}

	// ユーザー独自のキーイベント関数が登録されている場合
	if r.keyEvent != nil {
		gelog.Key("[KeyDispatch:user-function]",
			"id", id,
			"from", nodeName(r.current),
			"key", k,
		)

		keyStr, dispatchResult := r.keyEvent(ev)
		if dispatchResult == DispatchExecuted {
			r.appendKeyStatus(&KeyNode{KeyStr: keyStr})
			s := r.keyStatus
			// Reset
			r.keyStatus = ""
			r.current = r.root
			return s, DispatchExecuted
		}
	}

	node, ok := r.current.Next[k]
	if !ok {
		gelog.Key("[KeyDispatch:not-found]",
			"id", id,
			"from", nodeName(r.current),
			"key", k,
		)
		// 存在しないキーなら状態リセット
		result := DispatchNotFound
		if r.current != r.root {
			// 入力途中でキーが存在しなかった
			result = DispatchInvalidAfterPrefix
		}
		// 状態リセット
		r.keyStatus = ""
		r.current = r.root
		return r.keyStatus, result
	}

	gelog.Key("[KeyDispatch:found]",
		"id", id,
		"from", nodeName(r.current),
		"to", nodeName(node),
		"hasAction", node.Action != nil,
		"hasNext", len(node.Next) > 0,
	)

	if node.Action != nil {
		gelog.Key("[KeyDispatch:action]",
			"id", id,
			"key", node.KeyStr,
		)

		res := node.Action()

		gelog.Key("[KeyDispatch:action-result]",
			"id", id,
			"result", res,
		)

		if res == DispatchExecuted {
			r.Reset()
			gelog.Key("[KeyDispatch:reset]", "id", id)
		}
		return r.keyStatus, res
	}

	if len(node.Next) > 0 {
		gelog.Key("[KeyDispatch:prefix]",
			"id", id,
			"to", nodeName(node),
			"candidates", r.Candidates(),
		)

		// gelog.Key("プレフィックスキー → 次の入力待ち")
		// プレフィックスキー → 次の入力待ち
		r.current = node
		r.appendKeyStatus(node)
		return r.keyStatus, DispatchPrefix
	}

	// gelog.Key("到達不能だが安全策")
	// 到達不能だが安全策
	r.keyStatus = ""
	r.current = r.root
	return r.keyStatus, DispatchNotFound
}

func (r *RootNode) appendKeyStatus(n *KeyNode) {
	if r.keyStatus != "" {
		r.keyStatus += " "
	}
	r.keyStatus += formatMods(n.KeyStr)
	gelog.Key("KeyStatus", r.keyStatus)
}

func (r *RootNode) Candidates() []string {
	if r.current == nil || r.current.Next == nil {
		return nil
	}

	out := make([]string, 0, len(r.current.Next))
	for _, node := range r.current.Next {
		out = append(out, formatMods(node.KeyStr))
	}
	sort.Strings(out)
	return out
}

type binder struct {
	root *RootNode
	keys []string
}

// Bind 単一キーまたは階層キー列にアクションを登録
func (r *RootNode) Bind(keys ...string) *binder {
	return &binder{root: r, keys: keys}
}

func (b *binder) Do(fn func()) error {
	return b.root.bindKeys(b.keys, func() KeyDispatchTransition {
		fn()
		return DispatchExecuted
	})
}

// Non-blocking / pass-through
func (b *binder) DoAlso(fn func()) error {
	return b.root.bindKeys(b.keys, func() KeyDispatchTransition {
		fn()
		return DispatchNotFound
	})
}

type KeyAction func() KeyDispatchTransition

type KeyNode struct {
	Next   map[KeySpec]*KeyNode
	Action KeyAction
	KeyStr string
}

// newKeyNode 作成
func newKeyNode() *KeyNode {
	return &KeyNode{
		Next: make(map[KeySpec]*KeyNode),
	}
}

func formatMods(s string) string {
	repls := []struct {
		from string
		to   string
	}{
		{"ctrl+", "C-"},
		{"alt+", "M-"},
		{"shift+", "S-"},
		{"meta+", "Meta-"}, // or "M-" にするならここを調整
	}

	ls := strings.ToLower(s)
	for _, r := range repls {
		if strings.HasPrefix(ls, r.from) {
			return r.to + s[len(r.from):]
		}
	}
	return s
}
